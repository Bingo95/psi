...
var interface_content = function () {
	$(function () {
		supplylist_login(page_no);
	})
}
//列表加载，获取数据
function supplylist_login(page_no) {
	var _param = {};
	var p_data = {};
	...
	$.when(dfd)
		.done(function (data) {
			...
			if (_data.length == 0) {
				...
				$("#sum_totalmoney").text("总金额：" + data.Data.sum_totalmoney);
				//列表
				for (var i = 0; i < _data.length; i++) {
					cust_name = _data[i].cust_name; //供应商名字
					cust_con = _data[i].cust_con; //供应商地址
					cust_address = _data[i].cust_address; //供应商地址
					cust_tel = _data[i].cust_tel; //供应商电话
