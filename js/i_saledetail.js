var interface_content = function () {
	$(function () {
		$(".print_btn").attr("href", "i_saledetailprint.html?salecode=" + GetParameter("salecode"));
		detail_list();
	})
}
//列表加载，获取数据
function detail_list() {
	...
	$.when(dfd)
		.done(function (data) {
			...
			$("#cust_name").text(data.Data.cust_name + " " + data.Data.cust_address + " " + data.Data.cust_tel);
...
function other_click() {
	...
	$("#table_center tbody tr").unbind("click").click(function () {
		$("#table_center tbody tr").css("background-color", "#FFFFFF");
...
$("#out_saledetail").click(function () {
	...
	if (active == "i_salelist", "i_prodsalelist") {
		...
		window.location.href = "" + active + ".html?active=" + active + "&page_no=" + page_no + "&cust_id=" + cust_id + "&saledatef=" + saledatef +
			"&saledatet=" + saledatet + "&salecode2=" + salecode2 + add_parm;
	} else {
		window.location.href = "i_newsale.html?active=i_newsale";
	}
})

$("#edit_saledetail").click(function () {
	var active = GetParameter("active");
	if (active == "i_salelist", "i_prodsalelist") {
		var page_no = GetParameter("page_no");
		var salecode2 = GetParameter("salecode2");
		...
		window.location.href = "i_salemodify.html?active=" + active + "&salecode=" + GetParameter("salecode") + "&page_no=" + page_no +
			"&cust_id=" + cust_id + "&saledatef=" + saledatef + "&saledatet=" + saledatet + "&salecode2=" + salecode2 + add_parm;
	} else if (active == "i_newsale") {
		window.location.href = "i_salemodify.html?active=i_newsale&salecode=" + GetParameter("salecode");
	}
})
