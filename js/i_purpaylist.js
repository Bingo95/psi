...
function cust_select() {
    ...
    $.when(dfd)
        ...
        .done(function () {
            $(".searchable-select").css("left", "54px");
    ...
    $.when(dfd)
        .done(function (data) {
            ...
            if (_data.length == 0) {
                ...
                var paydate, paycode, paysort_value, cust_name, payothers, saleothers, operator, input_date, paymoney, purcode;
                //列表
                for (var i = 0; i < _data.length; i++) {
                    ...
                    listcontent.append(
                        ...
                        "<td style='font-size:14px'>" + input_date.substring(5) + "</td>" +
                        "<td>" +
...
function model_show(stat) {
    ...
    if (stat == 1) {
        ...
        $("#paysort").val("");
        $("#paymoney").val($("#moneyleft").text()).removeAttr("readonly");
        $("#payothers").val("").focus();
    } else if (stat == 0) {
        $("#save_pay").hide();
        $("#edit_pay").show();
...
var alert_model = function (_this) {
    ...
    $("#alert_text").text("您确定要删除支付记录【" + pay_code + "】吗？")
...
var detail_click = function (_this) {
    var purcode = $(_this).parents("tr").find("td").eq(4).text();
    ...
    window.location.href = "i_purdetail.html?active=i_purpaylist&page_no=" + page_no + "&purcode=" + purcode + "&cust_id=" +
